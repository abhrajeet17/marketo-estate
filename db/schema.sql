...
create table if not exists public.workspace_access (
  ...
  user_id uuid not null references auth.users(id) on delete cascade,
  access_type text not null check (access_type in ('client', 'broker', 'viewer')),
...
create index if not exists idx_workspace_access_user_workspace on public.workspace_access(user_id, workspace_id);
...
create index if not exists idx_crm_quote_templates_panorama on public.crm_quote_templates(panorama_id);
...
create index if not exists idx_crm_deal_quotes_status on public.crm_deal_quotes(status);
